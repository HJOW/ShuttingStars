...
class ShuttingStarsCore {
    ...
    timeMultiplier = 8.0;
    stageRows = 32; // 스테이지의 세로를 32등분하여 패턴의 시간과 매칭
    ...
    init(rootDiv) {
        ...
        const canvas = document.getElementsByClassName('shuttingstars_canvas_root')[0];
    ...
    handleKeyInput(key) {
        ...
        if(notePlacer == null) {
            ...
            if(key == 'ARROWUP' || key == 'ARROWDOWN' || key == 'ENTER') {
                ...
                return; // 해당 키에 맞는 NotePlacer 가 없는 경우, 무시
            }
        }
    ...
    displayResultMark(resultMark) {
        ...
        // 화면에 판정 결과 띄우기
        this.ctx.font = '20px Nanum Gothic Coding';
        if(resultMark == 'MISS') this.ctx.fillStyle = 'rgba(230, 80, 80, 0.8)'; // red
        else if(resultMark == 'BAD') this.ctx.fillStyle = 'rgba(230, 180, 80, 0.8)'; // purple
        else if(resultMark == 'GOOD') this.ctx.fillStyle = 'rgba(230, 230, 80, 0.8)'; // yellow
        else if(resultMark == 'GREAT') this.ctx.fillStyle = 'rgba(180, 230, 80, 0.8)'; // green
        else if(resultMark == 'PERFECT') this.ctx.fillStyle = 'rgba(80, 230, 80, 0.8)'; // blue

        let midX = this.resolution.w / 2;
        let midY = this.resolution.h / 2;
        this.ctx.fillText(resultMark, this.convertX(midX), this.convertY(midY)); // 화면 중앙에 출력

        // 콤보 띄우기
        if(this.combo > 1) {
            this.ctx.font = '15px Nanum Gothic Coding';
            this.ctx.fillStyle = 'rgba(230, 230, 230, 0.8)'; // white
            this.ctx.fillText('COMBO ' + this.combo, this.convertX(midX), this.convertY(midY + 30)); // 판정 결과 아래에 출력
        }
    }
    ...
    renderPlaying() {
        ...
        for(let idx=0; idx<this.objects.length; idx++) {
            ...
            if(obj instanceof NoteKeyObject) {
                if(obj.explosing >= 1 && obj.explosing < 10) {
                    obj.explosing++;
                }

                if((obj instanceof NotePlacer) && obj.explosing >= 10) {
                    obj.explosing = 0; // NotePlacer 는 애니메이션이 끝나면 다시 원래 상태로 돌아감
                }
            }
        }
    }
    ...
    timeElapse() {
        ...
        if(patternNow != null) {
            ...
            note.y = song.getNoteMoveSpeed() * this.stageRows; // 곡의 비트가 여기서 32번이 더 진행되었을 때 NotePlacer 에 도달하는 위치
        ...
        for(idx=0; idx<this.objects.length; idx++) {
            const obj = this.objects[idx];
            if((obj instanceof Note) && (obj.y < (obj.r / -2.0))) {
                if(obj.explosing == 0) {
                    // 미스 처리
                    let resultMark = 'MISS';
                    this.processResultMark(resultMark);
...
class NoteKeyObject extends ShuttingStarsObject {
    ...
    opacity = 1.0;
    dark = false;
    explosing = 0; // 0 : 일반적인 상황, 그 이상으로 가면 Note 제거 (혹은 Note 제거기 동작) 1~10 : 처리 애니메이션 진행상황
    ...
    getNowOpacity() {
        return this.opacity * (1.0 - (this.explosing / 10.0));
    }
    draw(ctx) {
        super.draw(ctx);
...
class NotePlacer extends NoteKeyObject {
    constructor(locationIndex) {
        super();
        this.r = 10;
        this.x = 20 + Math.round(locationIndex * this.r * 2.5);
        this.y = 40;
        this.key = _shuttingstarcore.keyList[locationIndex];
        this.shape = 'circle';
        this.opacity = 0.2;
...
class Note extends NoteKeyObject {
    ...
    constructor(locationIndex) {
        ...
        this.r = 10;
        this.x = 20 + Math.round(locationIndex * this.r * 2.5);
        this.y = _shuttingstarcore.resolution.h;
        this.key = _shuttingstarcore.keyList[locationIndex];
        this.shape = 'circle';
        this.opacity = 0.9;
        this.color = 'rgba(' + this.getColorOfLocationIndex(locationIndex) + ', ' + this.getNowOpacity() + ')';
